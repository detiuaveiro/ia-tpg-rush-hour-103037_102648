...
class State:
    ...
    def move(self, piece, direction):
        ...
        if direction[0]!=0 and (piece_coord[1]-piece_coord[0])!=1:
            return None
        ...
        if direction[0]==1: # move to the right
            if (piece_coord[-1]+1)%self.grid_size == 0: return None
            if self.get(piece_coord[-1]+1) != self.empty_tile: return None
        elif direction[0]==-1: # move to the right
            if (piece_coord[0]-1)%self.grid_size == self.grid_size-1: return None
            if self.get(piece_coord[0]-1) != self.empty_tile: return None
        elif direction[1]==1:
            if (piece_coord[-1]+self.grid_size) >= self.grid_size*self.grid_size: return None
            if self.get(piece_coord[-1]+self.grid_size) != self.empty_tile: return None
        elif direction[1]==-1:
            if (piece_coord[0]-self.grid_size)<0: return None
            if self.get(piece_coord[0]-self.grid_size) != self.empty_tile: return None
